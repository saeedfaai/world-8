...
Reference metadata was rechecked before final rendering, and the corrections were incorporated into manuscript v0.4, including official documentation access dates, the corrected Rodriguez author rendering, and venue-detail cleanup. The remaining pre-submission items are author-only confirmations for Funding and Competing Interests, final author approval, and provider-side submission/similarity screening.
