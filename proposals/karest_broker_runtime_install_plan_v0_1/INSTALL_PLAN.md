...
## Purpose
Define the exact gated path from the already-tested proposal chain to a reversible World 8 runtime candidate without silently crossing review, authority, secret, or Production boundaries.
...
All must have an explicit disposition bound to the exact head:
...
PENDING, silence, approval on another SHA, or a generic PR mergeability signal is not PASS.
...
3. Confirm no existing conflicting `service-karest-advisory-broker-001` Actor.
4. Confirm Groq route remains ACTIVE + VERIFIED using opaque credential reference only; never read/export raw provider secret.
5. Confirm broker policy remains advisory-only, provider-tools=false, consequential-effects=false, paid-fallback=false.
6. Confirm KAREST signer public-key enrollment design separately; do not generate or move a real private key in this plan.
7. Produce a pre-write evidence receipt containing exact heads, schema/writer fingerprints and zero/conflict counts.
...
### E. Install broker Edge candidate
- Only after Actor + Work + Workspace postchecks pass.
...
## Stop conditions
Immediately stop on: review mismatch, dependency SHA drift, actor identity conflict, inactive actor, schema/writer drift, route not VERIFIED, missing canonical Work/Workspace dependency, signature/policy widening, replay redispatch, secret exposure, provider-tool request, consequential-effect request, or paid fallback.
...
No World 8 runtime DDL, Actor/Work/Workspace write, Edge deploy, KAREST Production change, migration 046, signing-key generation, secret movement, provider call, merge, or FULL RELEASE PASS claim is performed by this file.
