...
begin
  ...
  v_repo:=coalesce(v_res.metadata->>'repo_ref',replace(v_res.provider_ref,'github:',''));

  if not v_pool.branch_protection_required then
    return jsonb_build_object('gate_state','PASS','protection_mode','POOL_NOT_REQUIRED');
  end if;
  if v_native in ('ENFORCED','RULESET_ENFORCED','PROTECTED') then
    return jsonb_build_object('gate_state','PASS','protection_mode','GITHUB_NATIVE');
  end if;

  if lower(coalesce(v_res.metadata->>'visibility',''))<>'private'
     or not coalesce((v_res.metadata->>'branch_protection_limitation_acknowledged')::boolean,false)
     or not coalesce((v_res.metadata->>'no_world9_git_write')::boolean,false) then
    return jsonb_build_object('gate_state','BLOCKED','reason_code','GITHUB_BRANCH_PROTECTION_REQUIRED');
  end if;
  ...
  if not found or v_check.status<>'PASS'
     ...
     or lower(coalesce(v_check.metadata->>'repo_ref',''))<>lower(v_repo) then
  ...
  return jsonb_build_object(
    'gate_state','PASS',
    'protection_mode','WORLD8_GOVERNED_PRIVATE_REPO_FALLBACK',
    'validation_check_id',p_validation_check_id,
    'head_commit',p_head_commit
  );
end$$;
